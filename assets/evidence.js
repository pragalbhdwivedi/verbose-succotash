(function(){
  const cards=[
    {
      ...
      meta:['Public website','GitHub-managed','Institutional operations'],sourceNote:'Live system + public repository'
    },
    {
      ...
      meta:['Prototype proposal','FET + HikCentral','Edge + central services'],sourceNote:'Formal prototype proposal available'
    },
    {
      ...
      url:'https://github.com/pragalbhdwivedi/k8s-ha-installer',meta:['Kubernetes','HA','Automation'],sourceNote:'Public repository'
    },
    {
      ...
      url:'https://github.com/pragalbhdwivedi/maas-configurations',meta:['MAAS','Cloud-init','Netplan','OVS'],sourceNote:'Public repository'
    },
    {
      ...
      meta:['Direct DC','PoE','Outdoor wireless','CCTV / NVR'],sourceNote:'Field engineering / system design'
    },
    {
      ...
      media:'./assets/media/identity-access/access-control_relay-troubleshooting_01.svg',
      meta:['COM / NC','Fail-safe lock','Exit input','Fault diagnosis'],sourceNote:'Technical wiring/troubleshooting record'
    },
    {
      ...
      url:'https://github.com/pragalbhdwivedi/aquapulse',meta:['Next.js','NestJS','PostgreSQL','RBAC'],sourceNote:'Public active-prototype repository'
    }
  ];

  function repoVisual(c,kicker,lines){return `<div class="evidence-media"><div class="repo-visual"><div class="repo-top"><span>${kicker}</span><span>Public GitHub</span></div><div class="repo-title">${c.title}</div><div class="repo-lines">${lines.map(x=>`<span>${x}</span>`).join('')}</div></div><div class="live-overlay"><span class="evidence-badge">${c.badge}</span><a class="evidence-open" href="${c.url}" target="_blank" rel="noreferrer">Repository ↗</a></div></div>`}
  function imageEvidence(c){return `<div class="evidence-media"><img src="${c.media}" alt="${c.title} evidence diagram" loading="lazy"/><div class="live-overlay"><span class="evidence-badge">${c.badge}</span><a class="evidence-open" href="${c.media}" target="_blank" rel="noreferrer">Open diagram ↗</a></div></div>`}
  function media(c){
    ...
    if(c.media)return imageEvidence(c);
  ...
  function cardAction(c,i){if(c.caseId)return `<button class="evidence-open evidence-case-action" data-evidence-index="${i}" style="border:0;cursor:pointer;margin-top:14px;align-self:flex-start">Open case study →</button>`;if(c.nodeId)return `<button class="evidence-open evidence-node-action" data-evidence-index="${i}" style="border:0;cursor:pointer;margin-top:14px;align-self:flex-start">Explore network node →</button>`;return ''}
  function wireActions(section){
    section.querySelectorAll('.evidence-case-action').forEach(btn=>btn.addEventListener('click',()=>{const c=cards[Number(btn.dataset.evidenceIndex)];if(c&&c.caseId&&typeof window.openCase==='function')window.openCase(c.caseId)}));
    section.querySelectorAll('.evidence-node-action').forEach(btn=>btn.addEventListener('click',()=>{const c=cards[Number(btn.dataset.evidenceIndex)];if(!c||!c.nodeId)return;if(typeof setMode==='function')setMode('explore');if(typeof expandPath==='function')expandPath(c.nodeId);window.scrollTo({top:0,behavior:'smooth'})}));
  }
  function build(){
    if(document.getElementById('evidenceRail'))return;const caseGrid=document.getElementById('caseGrid');if(!caseGrid)return;const section=document.createElement('section');section.className='evidence-section';section.id='evidenceRail';
    section.innerHTML=`<div class="evidence-head"><div><span class="eyebrow">Proof / evidence layer</span><h2>Architecture is useful. Evidence is better.</h2></div><p>Public systems and repositories are linked directly. Field and prototype work uses source-backed diagrams until privacy-safe project photography is ready.</p></div><div class="evidence-rail">${cards.map((c,i)=>`<article class="evidence-card">${media(c)}<div class="evidence-copy"><h3>${c.title}</h3><p>${c.body}</p><div class="evidence-meta">${c.meta.map(x=>`<span>${x}</span>`).join('')}</div>${c.sourceNote?`<div class="evidence-note">${c.sourceNote}</div>`:''}${cardAction(c,i)}</div></article>`).join('')}</div>`;caseGrid.insertAdjacentElement('afterend',section);wireActions(section)
  }
  function loadProofLayer(){if(document.querySelector('script[data-proof-layer]'))return;const script=document.createElement('script');script.src='./assets/proof.js';script.dataset.proofLayer='true';document.body.appendChild(script)}
  function init(){build();loadProofLayer()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();