(function(){
  ...
  let canvas=null,ctx=null,dpr=1,motes=[],lastFrame=0,decorateQueued=false,poweredUp=false,arrivalCount=0;
  ...
  function setGraphViewport(){svg.setAttribute('viewBox',mobile.matches?'250 40 900 820':'0 0 1400 900')}
  ...
  function animateArrival(node,delay){
    ...
    node.animate([
      ...
      {opacity:1,filter:'drop-shadow(0 0 14px rgba(186,255,103,.8))',offset:.5},
      {opacity:1,filter:'drop-shadow(0 0 0 rgba(186,255,103,0))'}
    ],{duration:540,delay,easing:'cubic-bezier(.16,.84,.28,1)'});
    setTimeout(()=>node.classList.remove('energizing'),delay+590);
  }

  function decorate(){
    decorateQueued=false;
    [...svg.querySelectorAll('.edge')].forEach((edge,i)=>{
      edge.classList.add('charged-edge');edge.setAttribute('pathLength','1');
      const key=edgeKey(edge),isNew=!seenEdges.has(key);seenEdges.add(key);
      if(isNew&&!reduced.matches)edge.animate([
        {opacity:0,filter:'drop-shadow(0 0 0 rgba(186,255,103,0))'},
        {opacity:1,filter:'drop-shadow(0 0 7px rgba(186,255,103,.72))',offset:.62},
        {opacity:edge.classList.contains('cross')?.35:.76,filter:'drop-shadow(0 0 0 rgba(186,255,103,0))'}
      ],{duration:430,delay:i*24,easing:'ease-out'});
    });
    [...svg.querySelectorAll('.node')].forEach((node,i)=>{
      ...
      if(isNew)animateArrival(node,80+i*38);
    });
  }
  ...
  function flashNode(id){
    const node=svg.querySelector(`.node[data-id="${CSS.escape(id)}"]`),base=node?.querySelector('circle:not(.node-halo)');if(!base||reduced.matches)return;
  ...
  function onGraphClick(event){const node=event.target.closest?.('.node');if(!node)return;requestAnimationFrame(()=>requestAnimationFrame(()=>zapNode(node.dataset.id)))}
  function pointerMove(event){const rect=shell.getBoundingClientRect();pointer.x=event.clientX-rect.left;pointer.y=event.clientY-rect.top;pointer.active=true}
  ...
  function init(){
    ...
    window.addEventListener('resize',()=>{setGraphViewport();resizeCanvas()},{passive:true});mobile.addEventListener?.('change',()=>{setGraphViewport();resizeCanvas()});
    reduced.addEventListener?.('change',()=>{if(reduced.matches){canvas?.remove();canvas=null;ctx=null}else if(!canvas)makeCanvas()});
    requestAnimationFrame(loop);window.__electricalMotion={zapNode,get arrivals(){return arrivalCount}};powerUp();
  }
