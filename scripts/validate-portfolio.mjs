...
const mustExist = [
  'index.html','CNAME','robots.txt','sitemap.xml','README.md','MASTER_ROADMAP.md','PORTFOLIO_STATUS.md','PORTFOLIO_ARCHITECTURE.md','PORTFOLIO_MAINTENANCE.md','RELEASE_CHECKLIST.md','CASE_REVIEW_REGISTER.md','CHANGELOG.md','SKILLS.md','EVIDENCE_REGISTER.md','OUTCOME_REGISTER.md','SPRINT_5_QUALITY_QA.md','SPRINT_6_DISCOVERABILITY_PERFORMANCE.md','assets/favicon.svg','assets/portfolio.js','assets/evidence.js','assets/proof.js','assets/case-navigation.js','assets/hiring-conversion.js','assets/accessibility.js','assets/network-navigation.js','assets/network-navigation.css','assets/case-review.js','assets/case-review.css'
];
...
for(const file of mustExist)if(!exists(file))fail(`Missing required file: ${file}`);
...
const portfolioCss=exists('assets/portfolio.css')?read('assets/portfolio.css'):'';
...
for(const snippet of requiredIndexSnippets)if(!index.includes(snippet))fail(`index.html missing required metadata/reference: ${snippet}`);
...
if(!jsonLdMatches.length)fail('No JSON-LD block found in index.html');else for(const[i,match]of jsonLdMatches.entries()){try{JSON.parse(match[1])}catch(error){fail(`JSON-LD block ${i+1} is invalid JSON: ${error.message}`)}}

function walk(dir){const out=[];for(const entry of fs.readdirSync(dir,{withFileTypes:true})){if(entry.name==='.git'||entry.name==='node_modules'||entry.name==='dist')continue;const full=path.join(dir,entry.name);if(entry.isDirectory())out.push(...walk(full));else out.push(full)}return out}
const allFiles=walk(root);
const jsFiles=allFiles.filter(f=>f.endsWith('.js')||f.endsWith('.mjs'));
for(const file of jsFiles){const rel=path.relative(root,file);try{execFileSync(process.execPath,['--check',file],{stdio:'pipe'})}catch(error){fail(`JavaScript syntax error in ${rel}: ${String(error.stderr||error.message).trim()}`)}}

const liveFiles=allFiles.filter(f=>{const rel=path.relative(root,f).replaceAll('\\','/');return rel==='index.html'||rel.startsWith('assets/')}).filter(f=>/\.(html|js|css|svg)$/i.test(f));
...
const forbiddenScalePhrases=['4 institutions','7 mapped classroom','11-15 classroom','11–15 classroom','4 buses + 6 vans','4 buses / 6 vans','4 buses and 6 vans'];
for(const phrase of forbiddenScalePhrases)if(liveText.toLowerCase().includes(phrase.toLowerCase()))fail(`Protected precise scale phrase reintroduced: ${phrase}`);
const phoneLike=liveText.match(/(?:\+?91[\s-]?)?[6-9]\d{9}/g)||[];
for(const raw of phoneLike){const digits=raw.replace(/\D/g,'');if(digits!=='9555877000'&&digits!=='919555877000')fail(`Unexpected public phone-like number found: ${raw}`)}
...
const refs=new Set();
for(const file of refSources){const text=fs.readFileSync(file,'utf8');for(const match of text.matchAll(localRefPattern))refs.add(match[0])}
for(const ref of refs){const rel=ref.replace(/^\.\//,'');if(!exists(rel))fail(`Broken local asset reference: ${ref}`)}
note(`Validated ${refs.size} local asset references`);

const requiredCaseIds=['aquapulse','digitalops','infra','kubernetes-ha','smartclass','solarcctv','leadership-recruitment','curriculum-assessment','compliance-documentation','institutional-operations','academic-scheduling','admissions-communication'];
...
for(const id of requiredCaseIds){if(!nav.includes(id))fail(`Case navigation registry is missing: ${id}`);if(!reviewLayer.includes(`'${id}'`))fail(`Case review layer is missing: ${id}`)}
const expectedReviewNames=['AquaPulse','BDSPS Digital Operations','Private Infrastructure','HA Kubernetes','Smart Classroom','Solar CCTV Edge','Teacher Recruitment & Evaluation','Curriculum & Assessment','Compliance & Documentation','Institutional Operations','Academic Scheduling & Execution','Admissions & Institutional Communication'];
for(const name of expectedReviewNames)if(!reviewRegister.includes(name))fail(`CASE_REVIEW_REGISTER.md missing case: ${name}`);
...
for(const term of attributionTerms)if(!readme.includes(term))fail(`README attribution boundary missing discipline: ${term}`);
